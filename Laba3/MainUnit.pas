...
begin
  ...
  AddTerm(Terms, GetTPT(('Связующий'), GetPPT(11)));
  AddPage(Terms[H(('Связующий'))]^.Next^.Pages, GetPPT(21));
  AddPage(Terms[H(('Связующий'))]^.Next^.Pages, GetPPT(31));

  AddTerm(Terms[H(('Связующий'))]^.Next^.SubTerm, GetTPT('Тренировки', GetPPT(11)));
  AddTerm(Terms[H(('Связующий'))]^.Next^.SubTerm, GetTPT('Мячи', GetPPT(21)));
  AddTerm(Terms[H(('Связующий'))]^.Next^.SubTerm, GetTPT('Приемы', GetPPT(31)));
  AddTerm(FindTerm(GetTPT('Приемы', nil))^.Next^.Result^.SubTerm, GetTPT(('Сброс'), GetPPT(31)));

  AddTerm(Terms,GetTPT(('Либеро'), nil));
  AddPage(Terms[H(('Либеро'))]^.Next^.Pages, GetPPT(55));
  AddPage(Terms[H(('Либеро'))]^.Next^.Pages, GetPPT(105));
  AddPage(Terms[H(('Либеро'))]^.Next^.Pages, GetPPT(155));

  AddTerm(Terms[H(('Либеро'))]^.Next^.SubTerm, GetTPT('Форма', GetPPT(155)));
  AddTerm(Terms[H(('Либеро'))]^.Next^.SubTerm, GetTPT('Рост', GetPPT(105)));
  AddTerm(Terms[H(('Либеро'))]^.Next^.SubTerm, GetTPT('Приемы', GetPPT(55)));
end;
...
begin
  upterm := InputBox('Добавление страницы', 'Введите термин : ', '');
  if upterm = '' then exit;
  ...
  if frez = nil then
    ShowMessage('Термин не найден!')
  else
    begin
      upterm := InputBox('Добавление страницы', 'Введите номер страницы: ', '');
      if upterm = '' then exit;

      AddPage(frez^.Pages,GetPPT(StrToInt(upterm)));
      ShowMessage('Страница успешно добавлена!');
    end;
end;
...
begin
  //Проверки ввода
  upterm := InputBox('Удаление термина/подтермина', 'Введите имя термина/подтермина : ', '');
  if upterm = '' then exit;
  ...
  //Запоминаем и искомый термин и результат поиска для получения родителя
  ...
  begin
    ...
    ShowMessage('Термин удалён успешно!');
  end;
end;
...
begin
  //Проверки ввода
  upterm := InputBox('Удаление страницы', 'Введите имя термина/подтермина : ', '');
  if upterm = '' then exit;
  ...
  if frez = nil then
    ShowMessage('Термин не найден!')
  //Удалить последнюю страницу нельзя
  else if frez^.Pages^.Next^.Next = nil then
    ShowMessage('Невозможно удалить единственную страницу!')
  else
  begin
    upterm := InputBox('Удаление страницы', 'Введите номер страницы : ', '');
    if upterm = '' then exit;
    ...
    if page=nil then
      ShowMessage('Страница не найдена!')
    else
    begin
      prev^.Next := page^.Next;
      ShowMessage('Страница успешно удалена!');
    end;
  end;
end;
...
begin
  //Проверки ввода
  upterm := InputBox('Изменение термина/подтермина', 'Введите имя термина/подтермина : ', '');
  if upterm = '' then exit;
  ...
  begin
    //Проверки ввода
    upterm := InputBox('Изменение термина/подтермина', 'Введите новое имя термина/подтермина : ', '');
    if upterm = '' then exit;
    ...
    // полного перехеширования удалим старый и добавим новый термин
    AddHT := GetParrent(findT);
    DeleteTerm(AddHT, frez);
    frez^.Word := upterm;
    AddTerm(AddHT, frez);

    ShowMessage('Термин успешно изменён!');
  end;
end;
...
begin
  //Проверки ввода
    upterm := InputBox('Изменение страницы', 'Введите имя термина/подтермина : ', '');
    if upterm = '' then exit;
  ...
  begin
    //Проверки ввода
    upterm := InputBox('Изменение термина/подтермина', 'Введите старый номер страницы : ', '');
    if upterm = '' then exit;
    ...
    if page = nil then
      ...
      begin
        //Проверки ввода
        upterm := InputBox('Изменение термина/подтермина', 'Введите новый номер страницы : ', '');
        if upterm = '' then exit;
        ...
        ShowMessage('Страница успешно изменёна!');
      end;
  end;
end;
