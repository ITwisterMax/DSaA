...
begin
  //вывод списка кандидата
  tempspis := HeadSpisok;
  if tempspis^.adrnext = headspisok then
    writeln('Список пуст')
  else
    begin
      writeln('Список абонентов и их номеров : ');
      writeln('-------------------------------------');
      writeln('|    Имя абонента    |   Телефон    |');
      writeln('|                    |              |');
      writeln('-------------------------------------');
...
begin
  ...
  i := 0;
  //пока не конец файла, читаем очередную строку
...
begin
  ...
  while tempspis^.adrnext <> headspisok do
    tempspis := tempspis^.adrnext;
  //выделяем память в конце списка
  new(tempspis^.adrnext);
  tempspis^.adrnext^.adrprev := tempspis;
  tempspis := tempspis^.adrnext;
  tempspis^.adrnext := headspisok;
  ...
  with tempspis^.inf do
    begin
      writeln('Добавление данных об абоненте : ');
      write('Номер : ');
      readln(phone_number);
      write('Название : ');
      readln(name);
...
begin
  tempnew := headnew;
  writeln('Полученный однонаправленный список : ');
  writeln('-------------------------------------');
  writeln('|    Имя абонента    |    Телефон   |');
  writeln('|                    |              |');
  writeln('-------------------------------------');
...
begin
  ...
  writeln('3.Создание однонаправленного списка |');
  writeln('4.Выход                             |');
  writeln('-------------------------------------');
...
begin
  //выделяем память для головы списка
  new(Headspisok);
